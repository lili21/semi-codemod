...
export default function transformer(file, api) {
  ...
  const dividerImportDeclaration = antdImportDeclaration.find(j.ImportSpecifier, {
    imported: {
      name: 'Divider'
    }
  });
  ...
  if (semiUiImportDeclaration.length) {
    ...
    semiUiImportDeclaration.get('specifiers').push(j.importSpecifier(j.identifier('Divider')));
  } else {
  ...
  // Find the JSX element <Divider /> has type attirbute
